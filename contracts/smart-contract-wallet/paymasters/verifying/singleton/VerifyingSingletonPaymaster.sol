...
contract VerifyingSingletonPaymaster is BasePaymaster {
    ...
    function deposit(address paymasterId) public payable {
        require(
            !Address.isContract(paymasterId),
            "Paymaster Id can not be smart contract address"
        );
        require(
            paymasterId != address(0),
            "Paymaster Id can not be zero address"
        );
    ...
    */
    ...
    function validatePaymasterUserOp(
        ...
        require(
            requiredPreFund <= paymasterIdBalances[paymasterData.paymasterId],
            "Insufficient balance for paymaster id"
        );
